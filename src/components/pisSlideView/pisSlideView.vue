  ...
  <div class="pis-slide-view">
    ...
    <el-dialog width="500px" :title="title" :visible.sync="labelChangeView"
        ...
        <el-row>
          <el-col :span="24">
            <el-form-item label="名称" label-width="40px" prop="name">
              <el-select v-if="labelType === 'edit'" size="mini"
              ...
              <el-input v-if="labelType === 'add'" size="mini"
          ...
          <el-col :span="24">
            <el-form-item label="描述" label-width="40px">
              <el-input size="mini" type="textarea" v-model="labelForm.description"
      ...
      <div slot="footer" class="dialog-footer">
        <el-button @click="closeLabelDialog" size="mini">取 消</el-button>
        <el-button type="primary" @click="saveLabelChange()" size="mini">确 定
        </el-button>
      </div>
    </el-dialog>
    ...
    <image-label-list ref="imageLabelList" :image-list="labelImages" @switch-picture="switchPicture"></image-label-list>
  </div>
</template>

<script>
import {pisSlideViewService} from './pisSlideView.service';
import {baseUrl, url, encodeQueryData} from '@/config/utils';
...
export default {
  name: 'pisSlideView',
  components: {ImageLabelList, FileChange, LabelInfo, HzztIcon},
  data() {
    return {
      kfbView: {},
  ...
  methods: {
    lastFile() {
      this.$emit('update:fileIndex', --this.fileIndex);
      this.viewDestroy();
      setTimeout(() => {
        this.init();
      }, 100);
    ...
    async initKfbView() {
      ...
      this.imageInfo.previewImg = `${baseUrl}/process/getImage?file=${this.fileName}&type=preview`;
      this.imageInfo.labelImg = `${baseUrl}/process/getImage?file=${this.fileName}&type=label`;
      ...
      this.kfbView = new KfbView({
        ...
        thumbnail: `${baseUrl}/process/getImage?file=${this.fileName}&type=thumbnail`,
        ...
        openSeadragonOptions: {
          maxZoomLevel: body.scanScale * 20,
          tileSources: {
            height: body.height,
            width: body.width,
            tileSize: 256,
            ...
            getTileUrl(level, x, y) {
              ...
              return `${url}/image.php?${encodeQueryData(params)}`;
            },
          },
        },
      });
    ...
    drawPointList() {
      return pisSlideViewService.navigatorPointList(this.fileName).then((res) => {
    ...
    pointListChange() {
      this.kfbView.addHandler('point-list-change', (e) => {
        pisSlideViewService.navigatorPoint(this.kfbView.navigator.pointList, this.fileName).then((res) => {
          this.kfbView.navigator.drawPointList(res.data.data);
        });
      });
    },
    drawLabelList() {
      return pisSlideViewService.labelList(this.fileName).then(({body}) => {
        this.initLabelList(body.data);
      });
    },
    labelImageList() {
      pisSlideViewService.labelImageList(this.fileName).then((res) => {
        this.labelImages = res.data.data;
      }).finally(() => {
      });
    },
    labelChange() {
      this.kfbView.addHandler('label-change', (e) => {
        ...
        switch (e.detail.type) {
          ...
          case 'move': {
            ...
            pisSlideViewService.modifyLabel(label.id, this.getShapeParams(label), this.fileName).then(({body}) => {
            });
          }
        }
      });
    ...
    saveLabelChange() {
      if (this.labelType === 'add') {
        ...
        this.kfbView.change();
        pisSlideViewService.saveLabel(this.getShapeParams(this.labelForm), this.fileName).then(({body}) => {
          this.initLabelList(body.data);
    ...
    deleteLabel(id) {
      pisSlideViewService.deleteLabel(id, this.fileName).then(({body}) => {
        this.initLabelList(body.data);
        this.labelChangeView = false;
      ...
      this.kfbView.addHandler('screen-image-change', (e) => {
        if (e.detail.type === 'done') {
          if (ClipboardItem && navigator?.clipboard) {
            const blob = this.dataURLtoBlob(e.detail.dataUrl);
    ...
    setMeasureInfo(draw) {
      ...
      if (draw.type === 'Measure') {
        ...
        this.infoList = [
          `宽: ${Math.abs(draw.width).toFixed(2)}微米`,
    ...
    initLabelList(data) {
      ...
      this.labelNameList = [
        ...new Set((data|| []).filter((item) => item.id && !item.content).map((item) => item.name))];
      this.kfbView.setLabelList((data || []).filter((item) => item.id && !item.content).map((item) => ({
        ...item,
    ...
    switchPicture(index) {
      ...
      const label = this.kfbView.labelList[index];
      if (!label) return;
      label.select = true;
