  ...
  <div class="main-container">
    <div class="common-header">
      ...
      <div class="common-header-end flex align-items-center">
        <!--<el-input style="margin-right: 22px" v-model="keyword" size="mini" clearable
                  placeholder="请输入用户名称"
    ...
    <div class="common-table">
      <el-table ref="pTable" v-loading="loading" :data="data" border :height="tableHeight" style="width: 100%">
        ...
        <el-table-column prop="role" label="角色" min-width="120" align="center">
          <template slot-scope="scope">
            <el-tag v-for="role in scope.row.role" :key="role.name" style="margin: 0 5px">{{role.name}}</el-tag>
          </template>
        </el-table-column>
        <el-table-column prop="role" label="所属分类" min-width="120" align="center">
          <template slot-scope="scope">
            <el-tag v-for="cate in scope.row.cate" :key="cate.cate_name" style="margin: 0 5px">{{cate.cate_name}}</el-tag>
          </template>
        </el-table-column>
    ...
    <el-dialog title="用户信息" :visible.sync="dialogUser" center width="40%">
      <el-form size="mini" :model="userForm" label-width="80px" label-position="left" class="edit-user-form" ref="userForm" :rules="userRules">
        ...
        <el-form-item label="分类" prop="cate">
          <el-select v-model="userForm.cate" multiple placeholder="请选择分类" style="width: 100%">
            <el-option v-for="item in classifies" :key="item.label" :label="item.label" :value="item.id">
            </el-option>
          </el-select>
        </el-form-item>
      ...
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" size="mini" @click="saveUser">保 存
        </el-button>
...
export default {
  ...
  methods: {
    ...
    editUser(user) {
      ...
      this.userForm = {
        ..._form,
        cate: (Array.prototype.concat.apply([], (_form.cate).map(d => d.id)))
      };
    },
    roleList() {
    ...
    cateList() {
      this.loading = true;
      userService.cateList().then(({ body }) => {
        if (body?.ret_code === 0) {
