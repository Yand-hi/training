<template>
  <div class="main-container role-main">
    ...
    <div class="flex">
      <div class="flex-2" style="margin-right: 24px">
        <el-row class="tree-page">
          <div class="flex">
              <!--
            <el-button type="primary" icon="el-icon-plus" size="mini" @click="addCate">添加</el-button>
          ...
          <el-table
            ...
            <el-table-column prop="operate" align="center" label="操作">
              <template slot-scope="scope">
                <el-button size="mini" type="primary" @click="editRole(scope.row)">编辑</el-button>
      ...
      <div class="flex-3">
        <el-row class="tree-page" v-if="roleRightForm.id">
          <el-form
            size="mini" label-position="left" label-width="80px" :model="roleRightForm"
            ref="classifyForm">
            ...
            <el-form-item>
              <el-button type="primary" size="mini" @click="editRole(roleRightForm)">编辑</el-button>
              <el-button type="danger" size="mini" @click="deleteRole(roleRightForm.id)">删除</el-button>
            </el-form-item>
          </el-form>
        </el-row>
      </div>
    ...
    <el-dialog title="新增角色" :visible.sync="dialogRole" center width="500px">
      <el-form
        size="mini" :model="roleForm" label-width="80px" label-position="left"
      ...
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" @click="saveRole" size="mini">保 存
        </el-button>
        <el-button @click="cancelRole" size="mini">取 消</el-button>
      </div>
    </el-dialog>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      saveRole() {
        ...
        if (this.roleForm.id) {
          roleService.saveRole({name: this.roleForm.name, cates: ids, id: this.roleForm.id})
            .then(({body}) => {
              if (body?.ret_code === 0) {
                this.cancelRole();
                this.roleList();
          ...
          roleService.addRole({name: this.roleForm.name, cates: ids,type:'扫描员'})
            .then(({body}) => {
              if (body?.ret_code === 0) {
                this.cancelRole();
                this.roleList();
