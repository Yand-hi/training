      ...
      <div class="folder-list flex justify-content-between align-items-center" :class="{'active-folder': item.id === activeTab}" v-for="(item, index) of menuList" :key="index" :label="item.name" @click="getData(item.id, $event)">
        <div class="folder-name flex justify-content-center align-items-center">
          <span v-if="item.editActive && item.id === activeTab">
            <el-input size="mini" ref="editActive" v-model="addMenuName" :key="item.id" class="edit-input" @blur="showMessage" :class="'edit-name'+index" style="width: 120px" @keypress.native.enter="createMenu(item.id,'2',$event)"></el-input>
          </span>
...
export default {
  ...
  methods: {
    ...
    getType() {
      testSetService.sliceType({ type: 1 }).then(({ body }) => {
        if (body.ret_code === 0) {
    ...
    createMenu(val, type, $event) {
      ...
      if (type === '1') {
        if (this.addMenuName === null) {
          ...
          testSetService.addMenu({ name: this.addMenuName, type: this.setType }).then(res => {
        ...
        testSetService.addMenu({ name: this.addMenuName, type: this.setType, id: val }).then(res => {
          if (res.body.ret_code === 0) {
    ...
    getMenu() {
      testSetService.getMenu().then(res => {
    ...
    getData(data) {
      ...
      testSetService.getFolderInfo(params).then(({ body }) => {
        if (body.ret_code === 0) {
      ...
      testSetService.sliceList({ folder_id: this.activeTab }).then(({ body }) => {
        if (body.ret_code === 0) {
    ...
    deleteFolder(data) {
      this.$confirm('是否删除该目录?', '提示', {
        ...
        testSetService.deleteMenu({ id: data }).then(({ body }) => {
          if (body?.ret_code === 0) {
    ...
    onSubmit() {
      ...
      testSetService.sliceList(params).then(({ body }) => {
        if (body.ret_code === 0) {
          this.data = (body.data.list).map((item) => ({
            ...item,
            slice_type: item.slice_type?.name
    ...
    removeSlice() {
      ...
      if (this.multipleSelection === undefined) {
        ...
        this.$confirm('是否从此目录移除该切片?', '提示', {
          ...
          testSetService.removeSlice({ slice_ids: (this.slice_id).toString(), folder_id: this.activeTab }).then(({ body }) => {
            if (body?.ret_code === 0) {
