<template>
  <div class="main-container entry-main">
    ...
    <div class="common-table" style="padding: 5px 10px;margin-top: 0">
      <div class="common-table-header flex justify-content-between align-items-center wrap" style="width: 100%;">
        <el-tabs v-model="activeTabName" @tab-click="handleClick">
          <el-tab-pane v-for="tab in tabList" :key="tab.name" :label="tab.label" :name="tab.name">
          </el-tab-pane>
        </el-tabs>
        <div class="flex" v-if="activeTabName === '待关联'">
          <el-input class="slice-name-input" style="margin-right: 12px" v-model="keyword" size="mini" clearable placeholder="请输入切片名称" suffix-icon="el-icon-search" v-on:keyup.enter.native="getSliceList()"></el-input>
          <el-select style="margin-right: 12px" v-model="source_id" size="mini" placeholder="请选择来源" clearable @change="getSliceList()">
            <el-option v-for="source in origins" :label="source.label" :value="source.id" :key="source.id"></el-option>
          </el-select>
          <el-button type="primary" size="mini" @click="autoScan" :disabled="isShowScanProgress">自动扫描</el-button>
          <el-button type="primary" size="mini" @click="patchRelateSource">批量来源关联</el-button>
          <el-button type="primary" size="mini" @click="patchRelate">批量病理号关联</el-button>
          <el-button type="primary" size="mini" @click="autoRelate">自动病理号关联</el-button>
        ...
        <div class="flex" v-if="activeTabName === '未知'">
          <el-input style="margin-right: 12px" v-model="keyword" size="mini" clearable placeholder="请输入切片名称" suffix-icon="el-icon-search" v-on:keyup.enter.native="getSliceList({status: 2})"></el-input>
          <el-button type="primary" size="mini" @click="loadJSON()">批量下载JSON</el-button>
          <!--<el-button type="danger" size="mini" @click="deleteSlice()">移除</el-button>-->
          <el-button type="danger" size="mini" @click="batchDelete()">批量删除</el-button>
        </div>
      </div>
    </div>
    <div class="slice-body flex">
      <div v-show="activeTabName !== '未知'" class="slice-tree" style="margin-right: 10px">
      ...
      <div class="slice-table common-table">
        <el-table ref="pTable" v-loading="tableLoading" :data="data" border :height=tableHeight style="width: 100%" @selection-change="handleSelectionChange">
          ...
          <el-table-column prop="operate" align="center" label="操作" fixed="right" width="180px">
            <template slot-scope="scope">
              ...
              <div v-if="activeTabName === '未知'" class="flex align-items-center justify-content-around wrap entry-template-btn">
                <el-button type="primary" size="mini" @click="readSlide(scope.row)">阅片</el-button>
    ...
    <el-dialog class="slice-pathological-dialog" title="病理详情" :visible.sync="pathologyFormVisible" center append-to-body>
    ...
    <el-dialog title="关联" class="entry-pathological-dialog" :visible.sync="entryFormVisible" center>
      <el-form :model="entryForm" label-width="120px" ref="entryForm" :rules="entryRules">
        ...
        <el-form-item label="数据来源" prop="source_id">
          <el-select v-model="entryForm.source_id" placeholder="请选择扫描仪来源">
            <el-option v-for="item in origins" :key="item.id" :label="item.label" :value="item.id">
            </el-option>
          </el-select>
        </el-form-item>
      ...
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" size="mini" @click="entryFormSubmit">确 定</el-button>
        <el-button size="mini" @click="entryFormVisible = false">取 消</el-button>
      </div>
    </el-dialog>
    <el-dialog title="关联来源" class="entry-source-dialog" :visible.sync="entrySourceFormVisible" center>
      ...
      <div slot="footer" class="dialog-footer">
        <el-button type="primary" size="mini" @click="entrySourceFormSubmit">确 定</el-button>
        <el-button size="mini" @click="entrySourceFormVisible = false">取 消</el-button>
      </div>
    </el-dialog>
...
export default {
  ...
  methods: {
    ...
    loadJSON() {
      ...
      entryService.downloadJSON(params).then(({ body }) => {
        if (body?.ret_code === 0) {
          let anchor = document.createElement('a');
